...
#Read in FTICR data
ft_mol <- read.csv("Data/ec1_water_fticrms_L2.csv", header=TRUE, sep=",")
ft_data <- read.csv("Data/ec1_water_fticrms_meta_L2.csv", header=TRUE, sep=",")
...
nmds.fortify<- ggvegan::fortify(nmds)
...
```
## Table of Anovas for Reviewer 1

```{r ANOVA table}
extract_anova_results <- function(anova_result, label) {
  
  summary_list <- summary(anova_result)
  summary_table <- do.call(rbind, lapply(summary_list, function(x) as.data.frame(x)))
  
  # Add rownames to the table as an "Effect" column so you can preserve the names
  summary_table <- tibble::rownames_to_column(summary_table, var = "Effect")
  
  # Add a column for the model label
  summary_table$Model <- label
  
  return(summary_table)
}

doc_table <- extract_anova_results(aov.doc, "DOC")
suva_table <- extract_anova_results(aov.suva, "SUVA")
cho_table <- extract_anova_results(aov.cho, "CHO")
chon_table <- extract_anova_results(aov.chon, "CHON")
chos_table <- extract_anova_results(aov.CHOS, "CHOS")

table_total <- rbind(doc_table, suva_table, cho_table, chon_table, chos_table)

table_clean <- table_total %>%
  dplyr::select(Model, everything())

write_csv(table_clean, "Data/ANOVA_results.csv")
...
## Reviewer 2 DOC vs Salinity Figure

```{r R2 figure}

doc_v_sal <-
  ggplot()+
    geom_point(data=data_final, aes(x=sal_psu, y=doc_mgC_L, color = region))+
    scale_color_manual(values=region_colors)+
    labs(x="Salinity (PSU)", y="DOC mgC/L")+
    theme_bw()+
    theme(axis.title.y=element_text(size=14, margin=margin(0,7,0,0)),
        axis.text=element_text(size=14),
...
ragg::agg_png("Figures/Fig_S5_DOC_v_Sal.png", width=6.5, height=6.5, units="in", res = 300, scaling=1)
print(doc_v_sal)
dev.off()
```
```{r R2 figure}
...
suva_v_sal <-
  ggplot()+
    geom_point(data=data_final, aes(x=sal_psu, y=SUVA254, color = region))+
    scale_color_manual(values=region_colors)+
    labs(x="Salinity (PSU)", y="SUVA 254")+
    theme_bw()+
    theme(axis.title.y=element_text(size=14, margin=margin(0,7,0,0)),
        axis.text=element_text(size=14),
        axis.text.x=element_text(angle=20, margin=margin(0,0,0,0), hjust=1),
